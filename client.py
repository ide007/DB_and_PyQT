            ...
            if self.database.check_user(edit):
                ...
                with sock_lock:
                    try:
                        ...
                        logger.error('Не удалось отправить информацию.')
        ...
        while True:
            ...
            with sock_lock:
                ...
                # Проблемы с соединением
                    ...
                    logger.critical(f'Потеряно соединение с сервером.')
                    break
                # Если пакет корректно получен выводим в консоль и записываем в
                # базу.
                else:
                    if ACTION in message and message[ACTION] == MESSAGE and \
                        ...
                        logger.error(f'Получено некорректное сообщение с '
                                     f'сервера: {message}')
...
# Функция разбирает ответ сервера на сообщение о присутствии, возвращает 200
# если все ОК или генерирует исключение при ошибке.
@log
...
# Функция инициализатор базы данных. Запускается при запуске, загружает данные
# в базу с сервера.
...
def main():
    ...
    try:
        ...
        answer = process_response_ans(read_message(transport))
        ...
        # Watchdog основной цикл, если один из потоков завершён, то значит или
        # потеряно соединение, или пользователь ввёл exit. Поскольку все
        # события обрабатываются в потоках, достаточно просто завершить цикл.
        while True:
            time.sleep(1)
